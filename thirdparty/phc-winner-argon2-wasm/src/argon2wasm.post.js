...
function wasm_argon2_hash(pwd,salt,modus,t_cost,m_cost, parallelism, size) {
    // verify parameters
    if ((!pwd) || typeof(pwd)!="string") throw new Error("Invalid parameters for argon2_hash: No password provided.");
    if ((!salt) || typeof(salt)!="string") throw new Error("Invalid parameters for argon2_hash: No salt provided");
    if ((modus===undefined) || (typeof(modus)!="number")) throw new Error("Invalid parameters for argon2_hash: No valid modus provided");
    if ((parallelism===undefined) || (typeof(parallelism)!="number")) throw new Error("Invalid parameters for argon2_hash: No parallelism provided");
    if ((t_cost===undefined) ||typeof(t_cost)!="number" ) throw new Error("Invalid parameters for argon2_hash: No time cost provided");
    if ((m_cost===undefined) || typeof(m_cost)!="number" ) throw new Error("Invalid parameters for argon2_hash: No memor cost provided");
    ...
    const returncode = _argon2_hash(t_cost, m_cost, parallelism,
        wasm_memory_password, pwd_length,
                wasm_memory_salt, salt_length,
                wasm_memory_hash, size,
                wasm_memory_encoded_hash, encoded_hash_length,
                modus, ARGON2_VERSION.ARGON2_VERSION_NUMBER);
    ...
    if (returncode!=0) {
        throw new Error(`Could not calculate argon2 hash. Error code: ${returncode}.`);
    }
...
function wasm_argon2_verify(hash,pwd,modus) {
    if ((!hash) || typeof(hash)!="string") throw new Error("Invalid parameters for argon2_verify: No hash provided or hash too short.");
    if ((!pwd) || typeof(pwd)!="string") throw new Error("Invalid parameters for argon2_verify: No password provided");
    if ((modus===undefined) || typeof(modus)!=="number") throw new Error("Invalid parameters for argon2_verifymake: No valid modus provided");
    ...
    const returncode = _argon2_verify(wasm_memory_hash, wasm_memory_pwd, pwd_length, modus);
