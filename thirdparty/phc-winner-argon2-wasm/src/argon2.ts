 ...
 */
export const argon2_min_pwd_length = 12;
export const argon2_min_salt_length = 16;
export const argon2_min_t_cost = 3; // 3 s
export const argon2_min_m_cost = 64 * 1024; // 2^N kibibytes, e.g. 2^15 = 32 mebibytesc
export const argon2_min_parallelism = 1;
export const argon2_min_size = 16; // default in blake2 is 64, for AES256 32 is needed
...
export class Argon2 {
  ...
  public argon2_hash(
    ...
    // check pwd
    if (pwd.length < argon2_min_pwd_length) {
      throw new Error(
        `Password of length ${pwd.length} does not correspond to minimal security settings.`
      );
    }
    // check salt
    if (salt.length < argon2_min_salt_length) {
      throw new Error(
        `Salt of length ${salt.length} does not correspond to minimal security settings.`
      );
    }
    // check size
    if (size < argon2_min_size) {
      throw new Error(
        `Size ${size} does not correspond to minimal security settings.`
      );
    }
    // check parameters
    ...
    for (let validatedParameter of validatedParameters) {
      if (
        ...
        if (
          ...
          throw new Error(
            `Parameter ${validatedParameter.parameterName} does not fulfill minimal security settings.`
          );
        }
      } else {
        switch (validatedParameter.parameterName) {
          ...
          default: {
            throw new Error(
              `Unknown Parameter ${validatedParameter.parameterName}`
            );
            break;
          }
        }
    ...
    return this.argon2Module.wasm_argon2_hash(
      pwd,
      salt,
      modus,
      t_cost,
  ...
  public argon2d_verify(hash: string, pwd: string): Argon2VerifyResult {
    return this.argon2Module.wasm_argon2_verify(
      hash,
      pwd,
      Argon2Modus.Argon2_d
    );
  }
  ...
  private validateParameters(
    ...
    if (parameters === undefined) {
      ...
      let t_cost_parameterResult: Argon2ParameterValidationResultType =
        Argon2ParameterValidationResultType.PARAMETER_INCOMPLETE;
      if (
        parameters.t_cost !== undefined &&
        parameters.t_cost >= argon2_min_t_cost
      ) {
      ...
      let m_cost_parameterResult: Argon2ParameterValidationResultType =
        Argon2ParameterValidationResultType.PARAMETER_INCOMPLETE;
      if (
        parameters.m_cost !== undefined &&
        parameters.m_cost >= argon2_min_m_cost
      ) {
      ...
      let parallelism_parameterResult: Argon2ParameterValidationResultType =
        Argon2ParameterValidationResultType.PARAMETER_INCOMPLETE;
      if (
        parameters.parallelism !== undefined &&
